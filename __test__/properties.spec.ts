 ...
 * Perf property: RUN_PERF_PROPERTIES=1 bun run test:props
 * NOT run in CI.
 */
...
const perfPropertyTest =
  process.env.RUN_PERF_PROPERTIES === "1"
    ? test
    : test.skip;
...
// the (?i) Unicode case folding bug). The absolute
// ceiling is the real guard; the relative ratio is
// only applied once the JS baseline is large enough
